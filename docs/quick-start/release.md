...
### v2.0-LZ-1stAnniv-dev.9.0

为迎合一周年连战活动制作的特殊版本。

下载链接如下：[v2.0-LZ-1stAnniv-dev.9.0](https://lzx0626.lanzouq.com/iLV0A3egmbeb)
...
2. 由于 BossRush V4 和风灵月影矛盾，因此实现了部分连战中经常需要用到得当功能作为风灵平替： 
   ...
   - Ctrl+F1 可以设置攻击力，指定数值请编辑 _atkBase_xxx 文件名；
   - Ctrl+F2 设置无限生命，_infHP_1 为开启，_infHP_0 为关闭；
   - Ctrl+F10 设置最大血量用于模拟半血出发夜叉头，对应文件 _maxHP_xxx；
   - Ctrl+F11 为设置棍势，对应文件 _gunshi_xxx；
   - Ctrl+F12 为设置四灾寒、火、毒、雷抗性，对应文件 _szXXX_xxx。
> 重要事项: 除棍势为一次性设置以外，其他都为常驻，需要编辑数值后再按下快捷键才可覆盖，其中 1_infHP_ 用 0,1 表示关闭和开启，其余都用 -1 表示关闭。每次修改完文件名后都需要按下对应快捷键保证生效。
3. 为了连战周年纪念赛修改了自定义连战中部分 boss 的血量，其中纪念赛老 boss 组（心中的不净、毒蛙、心中的寅虎、心中的黑手道人、心中的石猿、心中的二郎神（法相）、心中虎先锋 1 号）的血量和官方连战对齐（只有毒蛙需要修改），新 Boss 组（黑风、石先锋、莲眼、石蛙、赤尻马猴、璧水金晶兽），血量全部改为接近入圣难度的复战 boss 血量。
